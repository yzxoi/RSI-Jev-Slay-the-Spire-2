...
Validation-v5 (204eea8): 15/15 normal defeats and zero errors on the initial regression seeds, cost $0.029336790. However E012 heldout-v4 still stalls after Dense Vegetation heals (2 errors/30). The earlier prefix check only confirmed HP changed; that was insufficient to prove the event completed. New diagnosis: cosmetic NGame.ScreenRumble is called after audio Stop, but headless has no NGame receiver. Preserve the failed check. Next patch neutralizes only void ScreenRumble call sites in the copied DLL, consuming their arguments and retaining all original event logic. Require the event to advance to its fight option and actual combat before accepting the fix.

Final cosmetic fix a086be7: exact Dense Vegetation prefix now heals 9→30, exposes the new Fight option, and enters a four-Wriggler combat. Reporting initially used a missing enemy `id` key after the successful transition; corrected the report from its preserved trace, no replay of gameplay needed. All eleven focused tests pass. The immutable final headless DLL and assembly hashes are recorded in this fixture and E012 heldout-v5. This closes the event compatibility issue; gameplay-strength limits remain.
