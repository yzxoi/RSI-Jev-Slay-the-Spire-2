...
## Iteration 2 result and merge decision

Clean SHA `c9d91df`, 127 Python tests passed. The preregistered exact-response replay preloaded 617 requests, completed all 20 episodes as normal defeat and verified every raw trace hash. All 19 previously complete state/action transition sequences were identical; the old timeout reproduced all 351 existing transitions before continuing to a genuine Act 2 floor-12 defeat at step 373. Only three new Jev requests were required ($0.000698670). Batch wall time was 38.073 seconds, explicitly **cached replay**, not live-API speed evidence. E095 separately measured real-API serial 5.971s versus concurrent 1.810s.

Completed cohort progress: one deeper, three shallower, six tied; zero victories. Initial legacy-state parity is 10/10. Resource inventory never exceeded exported capacity. The original 558.875-second live-request batch and time-budget failure remain preserved. These are the same selected games, not 40 independent runs. New observation fields and resource choices change macro requests; the cohort does not isolate potion timing.

Decision: **merge the opt-in resource capability, not a stronger policy claim**. Real use/effects, normal discard/purchase with exact gold changes, pending reward replacement, automatic-only exclusion and selection continuation pass their fixed probes. Full-run execution completes under the integrated scheduler with exact trajectory reproduction, and the fresh API benchmark supports the runtime concurrency fix. Keep `retaliate` default unchanged. Natural probe coverage is narrow (Powdered Demise plus shop Weak Potion); synthetic Gambler/Fairy controls are labeled, not formal games. Old/new game-version parity remains E094, and no live MCP actions occurred (health endpoint was unavailable).

After merge, rebuild the main ignored runtime from the same pinned original DLL and run a one-state installation check (`e093_install_check`, Ironclad A10, resource mode, no model or game actions) to verify the installed binary exposes capacity. This is a deployment compatibility check only.
