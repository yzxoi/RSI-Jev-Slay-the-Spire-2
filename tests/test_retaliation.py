...
def state(block=0,hp=30,enemy_hp=100,hits=1,thorns=5):
 return {'energy':1,'player':{'hp':hp,'block':block},'enemies':[{'index':0,'hp':enemy_hp,'block':0,'powers':[{'name':'Thorns','amount':thorns}],'intents':[]}],'hand':[{'index':0,'name':'Strike','id':'CARD.STRIKE_IRONCLAD','type':'Attack','can_play':True,'cost':1,'target_type':'AnyEnemy','damage_by_target':[{'target_index':0,'damage':6,'repeat':hits,'total_damage':6*hits}]}]}
...
class RetaliationTests(unittest.TestCase):
 ...
 def test_power_damage_avoids_thorns(self):
  s=state();s['player']['powers']=[{'name':'Juggernaut','amount':8}];s['hand']=[{'index':0,'name':'Defend','id':'CARD.DEFEND_IRONCLAD','type':'Skill','cost':1,'can_play':True,'target_type':'Self','stats':{'block':5}}];f=forecast(s);self.assertEqual((f['predicted_self_loss'],f['predicted_block'],f['predicted_enemy_hp'][0]),(0,5,92))
 ...
 def test_block_before_attack_is_preferred_when_it_prevents_death(self):
  s=state(hp=4,enemy_hp=6);s['energy']=2;s['hand'].append({'index':1,'name':'Defend','id':'CARD.DEFEND_IRONCLAD','type':'Skill','cost':1,'can_play':True,'target_type':'Self','stats':{'block':5}});c,f=choose_plan(s,combat_candidates(s),triggers=True,retaliation=True);self.assertEqual(c['action']['args']['card_index'],1);self.assertEqual(f['predicted_self_loss'],0)
