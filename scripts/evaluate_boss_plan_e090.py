"""Exact-entry Boss room replay for four frozen Astra plans."""
...
PLANS_PATH = Path('experiments/E090/plans.json')
...
CONFIGS = [
    {'case_id': case['id'], 'arm': arm, 'character': case['character'],
     ...
     'policy': 'retaliate' if arm == 'baseline' else 'retaliate_boss_plan',
     ...
     **({'astra_boss_plan': PLANS_BY_CASE[case['id']]} if arm == 'treatment' else {})}
...
def trace_audit(path, case, arm):
    ...
    return {
        ...
        'plan_verified': ((len(plans) == 1 and plans[0] == PLANS_BY_CASE[case['id']])
                          if arm == 'treatment' else len(plans) == 0),
        'plan_decisions': plan_decisions,
        'plan_exposed': bool(plan_decisions) if arm == 'treatment' else not plan_decisions,
        ...
        'potion_budget_valid': len(potion_transitions) <= (PLANS_BY_CASE[case['id']]['max_potions'] if arm == 'treatment' else 0),
...
def main():
    ...
    for case in FIXTURE['cases']:
        ...
        pairs.append({'case_id': case['id'], 'character': case['character'],
                      ...
                      'baseline_cleared': bool(baseline.get('boss_cleared')),
                      'treatment_cleared': bool(treatment.get('boss_cleared')),
                      'baseline_max_round': baseline['max_round'],
                      'treatment_max_round': treatment['max_round'],
                      'baseline_last_enemy_hp': baseline['last_enemy_hp'],
                      'treatment_last_enemy_hp': treatment['last_enemy_hp'],
                      'treatment_potion_uses': treatment['potion_uses']})
    output = {'schema_version': 1, 'experiment': 'E090', 'manifest': manifest,
    ...
    Path('experiments/E090/result.json').write_text(json.dumps(output, ensure_ascii=False, indent=2) + '\n')
    print(json.dumps({'statuses': dict(Counter(run['status'] for run in results)),
                      ...
                      'baseline_clears': sum(p['baseline_cleared'] for p in pairs),
