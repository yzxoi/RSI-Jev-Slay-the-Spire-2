...
import json,uuid,time,hashlib,argparse
...
for iteration in range(2):
 ...
 try:
  state=h.send({'cmd':'load_save','path':str(ROOT/f'artifacts/private/native-floor7{suffix}.save'),'lang':'en'})
...
(ROOT/f'experiments/E005/load-{a.version}.json').write_text(json.dumps(report,indent=2)+'\n');print(json.dumps({k:v for k,v in report.items()if k!='manifest'},ensure_ascii=False))
