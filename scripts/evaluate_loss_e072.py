"""E072 preregistered paired full-run test of a stronger combat HP-loss penalty."""
...
SEEDS = [f'e072_loss_{n:03}' for n in range(1, 5)]
CHARACTERS = ['Ironclad', 'Silent']
ASCENSIONS = [0, 10]
POLICIES = ['retaliate', 'retaliate_hp3']
...
def main():
    ...
    paired = compare(results, 'retaliate', 'retaliate_hp3')
    output = {'schema_version': 1, 'experiment': 'E072', 'manifest': manifest,
              ...
              'same_state_action_differences': sum(result.get('same_state_action_differences', 0)
                                                   for result in results if result['policy'] == 'retaliate_hp3'),
    ...
    Path('experiments/E072/result.json').write_text(json.dumps(output, ensure_ascii=False, indent=2) + '\n')
