"""E084: matched A10 complete runs with card-reward deck-job context."""
...
SEEDS = [f'e084_jobs_{number:03}' for number in range(1, 5)]
CHARACTERS = ['Ironclad', 'Silent', 'Defect', 'Regent', 'Necrobinder']
BASELINE = 'retaliate'
TREATMENT = 'retaliate_deck_jobs'
CONFIGS = [
...
def trace_audit(path):
    ...
    with path.open(encoding='utf-8') as stream:
        for line in stream:
            ...
            elif kind == 'deck_job_summary':
                reward_summaries += 1
    return {'decisions': decisions, 'model_seconds': round(model_seconds, 3),
            'act1_boss_entries': len(boss_entries), 'reward_summaries': reward_summaries}
...
def main():
    ...
    for result in results:
        ...
        result['reward_summaries'] = detail['reward_summaries']
    ...
    Path('experiments/E084/result.json').write_text(json.dumps(output, ensure_ascii=False, indent=2) + '\n')
    print(json.dumps({
