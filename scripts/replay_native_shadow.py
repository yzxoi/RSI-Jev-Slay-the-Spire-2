...
for trial in range(2):
 ...
 try:
  ...
  for i,f in enumerate(source['frames']):
   ...
   if diff:
    prefix_only=set(diff)=={'hand'} and p['hand'][:len(expected['hand'])]==[tuple(x)for x in expected['hand']]
    comparisons[-1]['incomplete_native_hand_prefix']=prefix_only
    if not prefix_only:raise ValueError(f'Parity diverged before command {i}')
   s=h.send(f['command'])
...
report={'manifest':manifest,'results':results,'scope':'frozen observed combat projections only; unexported RNG/pile contents not yet checked'};(ROOT/'experiments/E005/replay-v2.json').write_text(json.dumps(report,indent=2)+'\n');print(json.dumps(results,ensure_ascii=False))
