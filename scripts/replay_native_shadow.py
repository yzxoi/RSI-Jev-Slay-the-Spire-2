...
manifest=version_manifest();assert not manifest['tracked_dirty'];os.environ['RSI_RESUME_NATIVE_ROOM']='1';source=json.loads((ROOT/'experiments/E005/prefix.json').read_text());results=[]
for trial in range(2):
 ...
 try:
  ...
  for i,f in enumerate(source['frames']):
   ...
   if diff:raise ValueError(f'Parity diverged before command {i}')
   s=h.send(f['command'])
  p=projection(s);expected=source['final_native_projection'];diff={k:{'native':v,'shadow':p.get(k)}for k,v in expected.items()if digest(v)!=digest(p.get(k))};comparisons.append({'step':len(source['frames']),'equal':not diff,'difference':diff});result['status']='passed'if not diff else'parity_mismatch';result['final_projection']=p
...
report={'manifest':manifest,'results':results,'scope':'frozen observed combat projections only; unexported RNG/pile contents not yet checked'};(ROOT/'experiments/E005/replay-v1.json').write_text(json.dumps(report,indent=2)+'\n');print(json.dumps(results,ensure_ascii=False))
