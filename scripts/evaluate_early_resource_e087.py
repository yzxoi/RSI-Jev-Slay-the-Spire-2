"""E087: matched A10 complete runs with one early potion decision per risky fight."""
...
SEEDS = [f'e087_resource_{number:03}' for number in range(1, 5)]
CHARACTERS = ['Ironclad', 'Silent', 'Defect', 'Regent', 'Necrobinder']
BASELINE = 'retaliate'
TREATMENT = 'retaliate_early_resource'
CONFIGS = [
...
def main():
    ...
    output = {
        'schema_version': 1, 'experiment': 'E087', 'manifest': manifest,
    ...
    Path('experiments/E087/result.json').write_text(json.dumps(output, ensure_ascii=False, indent=2) + '\n')
    print(json.dumps({
