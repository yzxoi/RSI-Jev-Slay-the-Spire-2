"""E075 preregistered paired full-run test of a stronger combat HP-loss penalty."""
...
SEEDS = [f'e075_weak_{n:03}' for n in range(1, 5)]
CHARACTERS = ['Ironclad', 'Silent']
ASCENSIONS = [0, 10]
POLICIES = ['retaliate', 'retaliate_weak']
...
def main():
    ...
    paired = compare(results, 'retaliate', 'retaliate_weak')
    output = {'schema_version': 1, 'experiment': 'E075', 'manifest': manifest,
              ...
              'same_state_action_differences': sum(result.get('same_state_action_differences', 0)
                                                   for result in results if result['policy'] == 'retaliate_weak'),
    ...
    Path('experiments/E075/result.json').write_text(json.dumps(output, ensure_ascii=False, indent=2) + '\n')
