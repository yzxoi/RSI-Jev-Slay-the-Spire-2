"""E086: matched A10 complete runs with previous-combat card-reward briefs."""
...
SEEDS = [f'e086_brief_{number:03}' for number in range(1, 5)]
CHARACTERS = ['Ironclad', 'Silent', 'Defect', 'Regent', 'Necrobinder']
BASELINE = 'retaliate'
TREATMENT = 'retaliate_combat_brief'
CONFIGS = [
...
def trace_audit(path):
    ...
    with path.open(encoding='utf-8') as stream:
        for line in stream:
            ...
            elif kind == 'combat_reward_brief':
                combat_reward_briefs += 1
    return {'decisions': decisions, 'model_seconds': round(model_seconds, 3),
            'act1_boss_entries': len(boss_entries), 'combat_reward_briefs': combat_reward_briefs}
...
def audit_pairs(results, trace_details):
    ...
    for seed in SEEDS:
        for character in CHARACTERS:
            ...
            valid_reward_divergence = (
                b is not None and t is not None
                and b['state_hash'] == t['state_hash']
                and b['decision'] == t['decision'] == 'card_reward'
            ...
            audit.append({
                ...
                'first_difference_valid_reward': valid_reward_divergence if first is not None else None,
...
def main():
    ...
    budget = Budget(max_calls=4000, max_usd=1.00, conservative_failures=True)
    ...
    for result in results:
        ...
        result['combat_reward_briefs'] = detail['combat_reward_briefs']
    paired = compare(results, BASELINE, TREATMENT)
    audit = audit_pairs(results, trace_details)
    output = {
        'schema_version': 1, 'experiment': 'E086', 'manifest': manifest,
    ...
    Path('experiments/E086/result.json').write_text(json.dumps(output, ensure_ascii=False, indent=2) + '\n')
    print(json.dumps({
        ...
        'reward_first_divergences': sum(a['first_difference_valid_reward'] is True for a in audit),
        'invalid_first_divergences': sum(a['first_difference_valid_reward'] is False for a in audit),
