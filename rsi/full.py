...
from .deck_context import deck_profile, deck_advice
...
def episode(config, manifest, jev=None):
    ...
    try:
        ...
        for step in range(config.get('max_steps',4000)):
            ...
            if d=='combat_play':
                ...
                elif config['policy'] in ['planned','planfixed','planfixed_cautious_route','planfixed_letter','triggered','retaliate','retaliate_deck_context']:
                    planning_state={**state,'skills_played_this_turn':skill_count} if config['policy']=='planfixed_letter' else state
                    selected,planning=choose_plan(planning_state,choices,triggers=config['policy'] in ['triggered','retaliate','retaliate_deck_context'],retaliation=config['policy'] in ['retaliate','retaliate_deck_context'],letter_opener=config['policy']=='planfixed_letter');trace.write('planning',planning)
                ...
                if config['policy'] not in ['hybrid','planned','jev','guarded','equipped','triggered','retaliate','retaliate_deck_context'] or len(choices)==1:
                    ...
                    if config['policy']=='retaliate_deck_context' and d in ('card_reward','shop'):
                        profile=deck_profile(state)
                        context.update(deck_profile=profile,deck_advice=deck_advice(profile))
                        trace.write('deck_profile',profile)
                        result['deck_profile_exposures']=result.get('deck_profile_exposures',0)+1
    ...
    context=state.get('context') or {}
    result.update(act=state.get('act',context.get('act')),floor=state.get('floor',context.get('floor')),final_decision=state.get('decision'),final_hp=state.get('player',{}).get('hp'),deck_profile_exposures=result.get('deck_profile_exposures',0),seconds=round(time.monotonic()-start,3),scenes=dict(scenes))
...
def main():
    ...
    if set(chars)-set(CHARACTERS) or set(policies)-{'first','greedy','hybrid','planned','planfixed','planfixed_cautious_route','planfixed_letter','jev','guarded','equipped','triggered','retaliate','retaliate_deck_context'}:p.error('Invalid character or policy')
    manifest=version_manifest()
    if manifest['tracked_dirty']:raise RuntimeError('Commit implementation before evaluation')
    budget=Budget(a.max_calls,a.max_usd,conservative_failures=True);jev=Jev(budget) if set(policies)&{'hybrid','planned','jev','guarded','equipped','triggered','retaliate','retaliate_deck_context'} else None
