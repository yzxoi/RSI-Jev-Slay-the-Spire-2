    ...
    try:
        ...
        for step in range(config.get('max_steps',4000)):
            ...
            if d=='combat_play':
                ...
                elif config['policy'] in ['planned','planfixed','planfixed_cautious_route','planfixed_letter','triggered','retaliate','retaliate_weak']:
                    planning_state={**state,'skills_played_this_turn':skill_count} if config['policy']=='planfixed_letter' else state
                    uses_retaliation=config['policy'] in ['retaliate','retaliate_weak']
                    selected,planning=choose_plan(planning_state,choices,triggers=config['policy'] in ['triggered','retaliate','retaliate_weak'],retaliation=uses_retaliation,letter_opener=config['policy']=='planfixed_letter',weak_forecast=config['policy']=='retaliate_weak');trace.write('planning',planning)
                    if config['policy']=='retaliate_weak':
                ...
                if config['policy'] not in ['hybrid','planned','jev','guarded','equipped','triggered','retaliate','retaliate_weak'] or len(choices)==1:
...
def main():
    ...
    if set(chars)-set(CHARACTERS) or set(policies)-{'first','greedy','hybrid','planned','planfixed','planfixed_cautious_route','planfixed_letter','jev','guarded','equipped','triggered','retaliate','retaliate_weak'}:p.error('Invalid character or policy')
    manifest=version_manifest()
    if manifest['tracked_dirty']:raise RuntimeError('Commit implementation before evaluation')
    budget=Budget(a.max_calls,a.max_usd,conservative_failures=True);jev=Jev(budget) if set(policies)&{'hybrid','planned','jev','guarded','equipped','triggered','retaliate','retaliate_weak'} else None
