...
def episode(config, manifest, jev=None):
    ...
    try:
        ...
        for step in range(config.get('max_steps',4000)):
                    ...
                    if plan_active(boss_plan,state):
                        ...
                        if turn_opener or followup:
                            if followup:
                                ...
                                history['boss_plan_turn']=turn_key
                            if decision_choices:
                                proposed=selected
                                selected,call=jev.choose({'state':model_state(state),'strategy':STRATEGY,
                                    'boss_plan':decision_context(boss_plan,state,planning,history.get('boss_potions_used',0))},
                                    decision_choices,trace)
                                result['model_calls']+=not call.get('cache_hit',False);result['cache_hits']=result.get('cache_hits',0)+call.get('cache_hit',False);result['cost_usd']+=call['usage'].get('cost',0)
                                trace.write('astra_boss_decision',{'state_hash':h,'turn_key':turn_key,
                                    'duplicator_followup':followup,'planner_choice':proposed,'choice':selected})
                                choices=decision_choices
                                if selected['action']['action']=='use_potion' and selected.get('name')=='Duplicator':
                                    history['duplicator_followup']=turn_key
            ...
            potions_before=(len((state.get('player') or {}).get('potions') or [])
                            if selected['action']['action']=='use_potion' else None)
            ...
            if potions_before is not None:
                ...
                trace.write('potion_transition',{'before':potions_before,'after':potions_after,
                    'used':potions_after==potions_before-1})
                if potions_after!=potions_before-1:raise RuntimeError('Potion action did not remove exactly one potion')
                history['boss_potions_used']=history.get('boss_potions_used',0)+1
                result['boss_potions_used']=history['boss_potions_used']
            if played_card:
            ...
            if config.get('stop_after_boss_room') and d in ('combat_play','card_select') and state.get('decision') not in ('combat_play','card_select'):
                result['boss_cleared']=state.get('decision')!='game_over'
                result['status']='boss_room_cleared' if result['boss_cleared'] else 'normal_defeat'
                trace.write('boss_room_boundary',{'status':result['status'],'state_hash':digest(state),
