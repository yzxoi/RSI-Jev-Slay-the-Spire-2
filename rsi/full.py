    ...
    try:
        ...
        for step in range(config.get('max_steps',4000)):
            ...
            if d=='combat_play':
                ...
                elif config['policy'] in ['planned','planfixed','rescue']:
                    selected,planning=choose_plan(state,choices);trace.write('planning',planning)
                    if config['policy']=='rescue':
                        selected,rescue=rescue_choice(state,choices,(selected,planning));trace.write('potion_rescue',rescue)
                        if rescue['override']:
                            result['potion_rescues']=result.get('potion_rescues',0)+1
                ...
                if config['policy'] not in ['hybrid','planned','jev','guarded','equipped','rescue'] or len(choices)==1: selected=fixed_macro(state,choices)
                else:
...
def main():
    ...
    if set(chars)-set(CHARACTERS) or set(policies)-{'first','greedy','hybrid','planned','planfixed','jev','guarded','equipped','rescue'}:p.error('Invalid character or policy')
    manifest=version_manifest()
    if manifest['tracked_dirty']:raise RuntimeError('Commit implementation before evaluation')
    budget=Budget(a.max_calls,a.max_usd,conservative_failures=True);jev=Jev(budget) if set(policies)&{'hybrid','planned','jev','guarded','equipped','rescue'} else None
