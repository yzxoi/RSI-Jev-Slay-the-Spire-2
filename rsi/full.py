    ...
    try:
        ...
        for step in range(config.get('max_steps',4000)):
            ...
            if d=='combat_play':
                ...
                elif config['policy'] in ['planned','planfixed','planfixed_cautious_route','planfixed_letter','triggered','retaliate','retaliate_hp3']:
                    planning_state={**state,'skills_played_this_turn':skill_count} if config['policy']=='planfixed_letter' else state
                    uses_retaliation=config['policy'] in ['retaliate','retaliate_hp3']
                    selected,planning=choose_plan(planning_state,choices,triggers=config['policy'] in ['triggered','retaliate','retaliate_hp3'],retaliation=uses_retaliation,letter_opener=config['policy']=='planfixed_letter',hp_loss_weight=3.0 if config['policy']=='retaliate_hp3' else 1.5);trace.write('planning',planning)
                    if config['policy']=='retaliate_hp3':
                        baseline,_=choose_plan(planning_state,choices,triggers=True,retaliation=True)
                        if baseline['id']!=selected['id']:
                            result['same_state_action_differences']=result.get('same_state_action_differences',0)+1
                            trace.write('same_state_action_difference',{'baseline':baseline,'treatment':selected})
                ...
                if config['policy'] not in ['hybrid','planned','jev','guarded','equipped','triggered','retaliate','retaliate_hp3'] or len(choices)==1:
...
def main():
    ...
    if set(chars)-set(CHARACTERS) or set(policies)-{'first','greedy','hybrid','planned','planfixed','planfixed_cautious_route','planfixed_letter','jev','guarded','equipped','triggered','retaliate','retaliate_hp3'}:p.error('Invalid character or policy')
    manifest=version_manifest()
    if manifest['tracked_dirty']:raise RuntimeError('Commit implementation before evaluation')
    budget=Budget(a.max_calls,a.max_usd,conservative_failures=True);jev=Jev(budget) if set(policies)&{'hybrid','planned','jev','guarded','equipped','triggered','retaliate','retaliate_hp3'} else None
