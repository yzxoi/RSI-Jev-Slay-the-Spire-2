    ...
    try:
        ...
        for step in range(config.get('max_steps',4000)):
            ...
            if d=='combat_play':
                ...
                elif config['policy'] in ['planned','planfixed','triggered','horizon']:
                    selected,planning=choose_plan(state,choices,triggers=config['policy'] in ['triggered','horizon'],horizon=config['policy']=='horizon');trace.write('planning',planning)
                else:
                    choices=computed_candidates(state,choices); selected=greedy_choice(state,choices)
            else:
                choices=macro_candidates(state,history)
                if config['policy'] not in ['hybrid','planned','jev','guarded','equipped','triggered','horizon'] or len(choices)==1: selected=fixed_macro(state,choices)
                else:
...
def main():
    ...
    if set(chars)-set(CHARACTERS) or set(policies)-{'first','greedy','hybrid','planned','planfixed','jev','guarded','equipped','triggered','horizon'}:p.error('Invalid character or policy')
    manifest=version_manifest()
    if manifest['tracked_dirty']:raise RuntimeError('Commit implementation before evaluation')
    budget=Budget(a.max_calls,a.max_usd,conservative_failures=True);jev=Jev(budget) if set(policies)&{'hybrid','planned','jev','guarded','equipped','triggered','horizon'} else None
