...
def main():
 p=argparse.ArgumentParser();p.add_argument('--run-id',required=True);p.add_argument('--action');p.add_argument('--reason');p.add_argument('--output');p.add_argument('--max-actions',type=int,default=2000);p.add_argument('--danger-hp',type=int,default=35);p.add_argument('--combat-policy',choices=['planned','jev'],default='planned');a=p.parse_args();manifest=version_manifest()
 ...
 if a.action:
  ...
  with (d/(pathlib.Path(a.output).stem+'.log')).open('w') as log:
   subprocess.run([sys.executable,'-m','rsi.campaign','--expected-run-id',a.run_id,'--execute','--combat-policy',a.combat_policy,'--expert-choice',str(expert),'--max-actions',str(a.max_actions),'--review-macro','--pause-on-danger','--danger-hp',str(a.danger_hp),'--stop-file','/tmp/rsi-sts2-native.pause','--output',a.output],cwd=ROOT,stdout=log,stderr=subprocess.STDOUT,check=True)
